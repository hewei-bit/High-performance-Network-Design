...
int ikcp_send(ikcpcb *kcp, const char *buffer, int len)
{
    ...
    if (len <= (int)kcp->mss)
        count = 1; // 分片(头部24) + user data(mss 1376) = mtu （1400）
    else
        count = (len + kcp->mss - 1) / kcp->mss;

    if (count >= (int)IKCP_WND_RCV)
        return -2; // 超过对方的初始接收窗口
...
void ikcp_flush(ikcpcb *kcp)
{
    ...
    if (kcp->nocwnd == 0)
        cwnd = _imin_(kcp->cwnd, cwnd); // 进一步控制cwnd大小
    ...
    for (p = kcp->snd_buf.next; p != &kcp->snd_buf; p = p->next)
    {
        ...
        { // 2 当前时间达到了重发时间，但并没有新的ack到达，出现丢包, 重传
            needsend = 1;
            segment->xmit++;
            kcp->xmit++;
            if (kcp->nodelay == 0)
        ...
        { // 3 segment的累计被跳过次数大于快速重传设定，需要重传
